"""
  Generated by Eclipse Cyclone DDS idlc Python Backend
  Cyclone DDS IDL version: v0.11.0
  Module: geometry_msgs.msg.dds_
  IDL file: QuaternionStamped_.idl

"""

from enum import auto
from typing import TYPE_CHECKING, Optional
from dataclasses import dataclass

import cyclonedds.idl as idl
import cyclonedds.idl.annotations as annotate
import cyclonedds.idl.types as types

# root module import for resolving types
import communicator.idl.geometry_msgs

if TYPE_CHECKING:
    import communicator.idl.std_msgs.msg.dds_



@dataclass
@annotate.final
@annotate.autoid("sequential")
class QuaternionStamped_(idl.IdlStruct, typename="communicator.idl.geometry_msgs.msg.dds_.QuaternionStamped_"):
    header: 'communicator.idl.std_msgs.msg.dds_.Header_'
    quaternion: 'communicator.idl.geometry_msgs.msg.dds_.Quaternion_'


